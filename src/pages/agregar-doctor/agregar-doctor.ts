...
import { IonicPage, NavController, LoadingController, NavParams, ToastController, AlertController, ViewController } from 'ionic-angular';
...
@IonicPage()
@Component({
  selector: 'page-agregar-doctor',
  templateUrl: 'agregar-doctor.html',
})
export class AgregarDoctorPage {
  ...
  ngOnInit(){
    this.doctorForm = new FormGroup({
      ...
      'nro_colegiatura': new FormControl(""),
    ...
    loading.present().then(_=>{
      this.database.existeTelefonoRegistrado(telefonoCompleto).then(async dataUsuario=>{
        if (dataUsuario) {
          ...
          let codigoDoctor=this.database.createId ();
          this.database.registrarDoctorCompletoConID(codigoDoctor, this.capitalizeFirstLetter(value.nombres), this.capitalizeFirstLetter(value.apellidos), value.email, value.dni, telefonoCompleto, value.nro_colegiatura, value.especialidades).then(_=>{
            loading.dismiss().then(()=>{
              this.presentToast("El doctor fue registrado correctamente","exito");
              // this.navCtrl.pop ();
              let data:any={
